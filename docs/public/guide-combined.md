...
Version 2026-08 is an update of version 2019-04[^1], originally authored by Olivier Dupriez, and updated over time by Diana Marcella Sanchez Castro and Matthew Welch (The World Bank). Their contributions to the original structure, content, and conceptual development of the current version are gratefully acknowledged.

The current version was revised and updated by Cathrine Machingauta and Mehmood Asghar to include the following revisions: 
- Reference to new tools developed by the World Bank which replace previous tools mentioned in the original version; 
- Reference to DDI-Codebook which is now an ISO standard, updated from DDI 1.6 referenced in the original version
- Updates to the following sections: 
    - introduction; 
    - folder organization incorporate World Bank file naming convention; 
    - preparing data
    - documenting the study description;
    - document description; 
    - generating output;   
    - quality review and an updated quality review checklist; 
    - addition of the Publishing to NADA and Admin metadata documentation sections. 
...
| Identification No | `IDNo` | Unique string or number.<br><br>**Guidance:** Use a consistent ID scheme.<br>The `IDNo` element is repeatable and can additionally be used to capture the **Digital Object Identifier** *(DOI)* with attributes `agency`="DOI" and `isPersistentIdentifier`="true" to set a persistent identifier.| **WLD_2023_SYNTH-SVY-EN_v01_M** |
...
By adopting these practices and leveraging open-source tools such as **sdcMicro**, the **Metadata Editor**, and **NADA**, data producers and curators can create high-quality, standards-compliant metadata that supports long-term preservation, facilitates data discovery, and maximizes the value and impact of data assets.

Ultimately, well-documented data are more than a record of what was collected. They provide the context, transparency, and evidence needed for users to understand, trust, reproduce, and reuse data effectively, ensuring that valuable data resources remain accessible and meaningful for years to come.
